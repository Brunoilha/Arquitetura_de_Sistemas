...
async function runMigrations() {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      console.log(`Running prisma migrate deploy for payments (attempt ${attempt}/${MAX_RETRIES})`)
...
async function runDbPush() {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      console.log(`Running prisma db push for payments (attempt ${attempt}/${MAX_RETRIES})`)
      const { stdout, stderr } = await execAsync('npx prisma db push --accept-data-loss', { env: process.env })
      if (stdout) console.log(stdout)
      if (stderr) console.error(stderr)
      console.log('Payments schema pushed successfully (db push)')
      return
    } catch (err) {
...
(async () => {
  ...
  try {
    ...
    console.error('Failed to start payments app after migrations:', err)
    process.exit(1)
  }
})()
